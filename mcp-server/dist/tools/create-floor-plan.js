...
function generateWallElements(placements, style) {
    const elements = [];
    ...
    for (const p of placements) {
        ...
        if (rightX < maxX) {
            elements.push({ type: "line", x1: rightX, y1: p.y, x2: rightX, y2: bottomY, color: style.wallColor, strokeWidth: 2 }, { type: "line", x1: rightX + WALL_THICKNESS, y1: p.y, x2: rightX + WALL_THICKNESS, y2: bottomY, color: style.wallColor });
        }
        if (bottomY < maxY) {
            elements.push({ type: "line", x1: p.x, y1: bottomY, x2: rightX, y2: bottomY, color: style.wallColor, strokeWidth: 2 }, { type: "line", x1: p.x, y1: bottomY + WALL_THICKNESS, x2: rightX, y2: bottomY + WALL_THICKNESS, color: style.wallColor });
        }
        // Room label (centered)
        elements.push({
            type: "text",
...
function generateDoorElements(placements, style) {
    const elements = [];
    for (const p of placements) {
        if (!p.room.doors)
            continue;
...
function generateWindowElements(placements, style) {
    const elements = [];
    for (const p of placements) {
        if (!p.room.windows)
            continue;
...
function generateDimensionElements(placements, scale, style) {
    const elements = [];
    ...
    elements.push({ type: "dim", x1: minX, y1: minY - 25, x2: maxX, y2: minY - 25, label: `${totalW} m`, color: style.dimColor }, { type: "dim", x1: minX - 25, y1: minY, x2: minX - 25, y2: maxY, label: `${totalH} m`, color: style.dimColor });
    return elements;
}
...
//# sourceMappingURL=create-floor-plan.js.map