...
- [x] **Wall orientation toggle for doors & windows in sidebar** — When any `window`, `opening`, `door-symbol`, or `door-slide` element is selected, a new "Wall orientation" section appears in the Selection panel with two buttons: "— Horizontal" (wall runs left–right) and "| Vertical" (wall runs up–down). Previously, toggling orientation between H and V required editing the raw `wall` field in JSON — the single most common post-placement operation on architectural elements, now just one click. Also fixed the column element coordinate editor which was showing incorrect `x`/`y`/`w` fields (column uses `cx`/`cy`/`s`); it now correctly shows those three fields matching the `ColumnElement` interface and the renderer. Works on both `/` and `/editor` pages. Build passes, zero-delta bundle size.


- [x] **Rotation editor in sidebar selection panel** — When any single element is selected, the `ElementCoordEditor` in the Selection panel now shows a dedicated "Rotation" section with: (1) a numeric input showing the current angle in degrees (editable — type any value and press Enter or click away to commit); (2) a "↺ reset" button that appears when rotation ≠ 0° to snap back to 0°; (3) six quick-preset buttons: 0°, 45°, 90°, 135°, 180°, 270°. All rotation changes go through `updateElement()` → `pushHistory()` so they are fully undoable/redoable (Ctrl+Z). Previously rotation was only accessible via R key (15° steps) or the drag handle — both requiring multiple steps to reach common angles like 90°. Works on all element types (line, rect, circle, arc, text, dim, furniture, etc.) on both `/` and `/editor` pages. Build passes, zero bundle size change.

- [x] **Layer visibility respected in Canvas hit-testing** — Elements assigned to a hidden layer were still fully interactive (click-to-select, drag-to-move, double-click, box-select, touch-tap, cursor hover) even though they were invisible on the canvas and in SVG export. The Canvas rendered floating selection boxes for hidden-layer elements that remained selected after a layer was hidden. Fixed: computed `hiddenLayers: Set<string>` (memoized via `useMemo` on `sketch.layers`) in Canvas.tsx; passed it to `hitTest()` and `boxHitTest()` as an optional 4th/6th argument so both functions return `false` for any element whose `layer` id is in the hidden set; all 8 call sites updated (click selection loop, double-click, mousedown drag-start, hitOnAny check, box-select on mouseup, touch drag, touch tap, cursor hover). Also suppressed the selection highlight drawing for hidden-layer elements so floating blue boxes no longer appear when a layer is toggled off with selected elements still on it. `useMemo` import added; all affected `useCallback` dependency arrays updated. Build passes, zero-delta bundle size.

- [x] **Furniture & architectural element property editing in sidebar** — All 11 furniture/fixture element types (bed, sofa, dining-table, armchair, desk, bookshelf, stove, shower, toilet, bathtub, sink) previously had *no* coordinate editor in the Selection panel — any position or size change required editing raw JSON. Fixed: all furniture elements now show x/y/w/h fields when selected. Also fixed `stair` (was missing `h`), `arc` (was missing `startAngle`/`endAngle`). Added type-specific property controls: `bed` → Single/Double toggle; `dining-table` → chairs-per-side (1/2/3); `bookshelf` → shelf count (2/3/4/5); `stove` → burner count (2/4); `stair` → steps input (2–20); `door-symbol` → swing direction (Left ↺ / Right ↻); `door-slide` → slide direction (← Left / → Right). All changes go through `updateElement()` → full undo/redo. Works on both `/` and `/editor` pages (desktop sidebar + mobile bottom-sheet). Build passes.
...
- [x] **canvasControlRef on /editor + fit-view command palette fix + preset label overlap** — (1) The `/editor` page was missing `canvasControlRef`, so pressing "Fit View" from the Ctrl+K command palette on that page was a silent no-op (the `fit-view` case in `handleCommandPaletteAction` had no target to call `resetView()` on). Fixed: `useRef` import added, `canvasControlRef` created and passed to `<Canvas canvasControlRef={canvasControlRef} />`, and `case "fit-view": canvasControlRef.current?.resetView()` added to the editor's palette handler. Now Fit View works identically on both `/` and `/editor`. (2) Fixed longstanding room label vs. furniture overlap in the two flagship floor plan presets: in "plan drawing", the KÖK label was at y=195, sitting inside the dining table (y=175..265), and VARDAGSRUM label at y=475 was inside the coffee table rect (y=435..495). In "villa", KÖK at y=210 was inside dining table (y=195..295) and VARDAGSRUM at y=510 was inside coffee table (y=480..550). All four labels moved to clear floor space (KÖK → below dining table, VARDAGSRUM → bottom of room near the bottom wall). These overlaps were visible on the very first load of the app's default preset — a poor first impression now resolved. Build passes, /editor bundle +0.04 kB.



- [x] **CommandPalette (Ctrl+K) wired into both / and /editor pages** - The CommandPalette component had been imported and had state created in `page.tsx` but was never rendered in the JSX — meaning pressing Ctrl+K opened nothing. Additionally, the palette was completely absent from `editor/page.tsx`. Fixed: (1) `<CommandPalette>` is now rendered in `page.tsx` JSX with all props wired (onLoadPreset, onAddElement, onSetPaper, onSetTool, onSetRenderStyle, onSetAmplitude, onSetSnapGrid, onAction); (2) `editor/page.tsx` now imports `CommandPalette`, adds `showCommandPalette` state, `handleCommandPaletteAction` callback (undo, redo, select-all, delete, duplicate, copy, paste, group, ungroup, new-sketch, select-same-type, redraw, download-png, download-svg, download-json, copy-json, print, save), Ctrl+K keydown handler, and renders `<CommandPalette>` in the JSX. Both pages now offer identical Ctrl+K palette functionality. Build passes, /editor bundle +0.29 kB.




- [x] **Network topology preset + dim offset fixes + G key grid snap toggle** — (1) New **"network topology"** preset: a full production infrastructure diagram with internet/cloud bubble, load balancer, 3 web servers, 2 app servers, PostgreSQL primary + read replica, Redis cache, Prometheus monitoring scraper, and a dashed DMZ firewall boundary rect. Uses solarized palette (blue=load balancer, cyan=web servers, yellow=app servers, red=postgres, orange=redis, green=monitoring). Legend at bottom-left. Wired into `/create` page matcher (keywords: network, topology, infrastructure, cloud, load balancer, kubernetes, microservice, api gateway, vpc) with 🌐 emoji. (2) **Vertical dim `offset: -20` fix** in villa, office layout, and garden plan presets — all three had their vertical dimension lines missing `offset: -20`, causing the dim line and extension lines to render inside the building footprint instead of cleanly outside the left wall. This was a longstanding visual quality bug noted in SKETCHAI_TODO.md. All three now match plan drawing and blueprint presets. (3) **G keyboard shortcut** for grid snap toggle — pressing `G` (no modifier) now toggles 20px grid snap on/off on both `/` and `/editor` pages. `Ctrl+G` (group) is unaffected. Shortcuts panel updated.



- [x] **SVG renderer dim offset fix — dashed extension lines + perpendicular offset** — The SVG renderer's `dim` element case completely ignored the `offset` property that the canvas renderer (`renderer.ts`) fully supports. This caused all SVG exports (Download SVG button, `/api/render` endpoint, share page, dashboard thumbnails) to render dimension lines at raw element coordinates — i.e., inside walls instead of outside them in floor plans. Fixed by mirroring the canvas renderer's dim logic exactly: (1) compute perpendicular direction vector; (2) apply offset to get shifted endpoints; (3) draw dashed extension lines (stroke-dasharray 3,3) from original points to offset positions; (4) draw main dim line and tick marks at offset positions; (5) place label at the offset midpoint (vertical dims: label offset 16px perpendicular). All SVG output paths now match canvas output for sketches that use `offset` on dim elements (all floor plan presets).

- [x] **Blueprint preset overhaul — proper double-line walls, full furniture, structural columns** — The blueprint preset was the only one of 8 presets that used simple `rect` boxes to represent rooms, making it look like a rough draft compared to the polished plan-drawing and villa presets. Complete rebuild: (1) **Double-line exterior walls** — outer + inner wall lines (8px gap, 2.5px stroke) on all 4 sides, same architectural convention as plan-drawing/villa; (2) **Double-line interior walls** dividing 6 rooms: Bedroom 1, Kitchen, Living Room, Master Bedroom, Bathroom, Hall; (3) **5 windows** on all 4 facades (top, left, right) with correct `#a8c8ff` blueprint blue-white color; (4) **4 doors** with correct swing directions; (5) **Stair** in hall (9 steps); (6) **Full furniture** in every room — double bed + nightstand (Bedroom 1), stove + dining table (Kitchen), sofa + armchair + coffee table (Living Room), double bed + nightstand + wardrobe (Master Bedroom), toilet + sink + bathtub (Bathroom); (7) **Structural column** at the main wall junction; (8) **Title block metadata** fully populated (title, owner, scale, sheetNumber, drawnBy); (9) **Dimension lines** (15.6m × 10.6m) with `offset: -20` on the vertical dim to push it cleanly outside the left wall. Blueprint palette properly layered: `#d8eaff` (walls), `#a8c8ff` (windows/doors), `#8ab8e8` (furniture/dims). Build passes, pushed.



- [x] **Enhanced public sketch share page — PNG/SVG download, redraw, embed code** — When a user opens a public sketch share link (`/s/<slug>`), the page previously showed only the canvas with a "Fork this sketch" header button and no way to download, share further, or embed. Now fully featured: (1) **Download PNG** button (canvas.toBlob → direct browser download, Ctrl+Shift+S keyboard shortcut); (2) **Download SVG** button (uses the full `renderSketchToSVG` renderer for crisp vector output, Ctrl+Shift+E); (3) **Redraw** button — re-renders with a new random wobble seed so each redraw produces a unique hand-drawn variation (R key shortcut); (4) **Copy Link** button — copies the public `/s/<slug>` URL to clipboard; (5) **Embed** panel — togglable code block with a ready-to-paste `<iframe>` snippet for embedding on any website. A keyboard hint is shown below the canvas. `PublicSketchView.tsx` now accepts `slug` and `title` props (passed from `page.tsx`) for correct URL construction and smart filenames. TypeScript clean, build passes at 2.32 kB (+1.84 kB over old 479B stub). Turns the share page from a dead-end into a full-featured viewer.



- [x] **Smart /create page — keyword preset matcher + editor redirect** — The `/create` page previously showed a "coming soon" dead-end after every prompt submission. Now fully wired: `matchPreset()` scores the user's prompt against 8 keyword-rule sets (plan drawing, villa, blueprint, office layout, flowchart, architecture, garden plan, napkin sketch) and selects the highest-scoring preset. When the user clicks "Create Sketch", a green confirmation card shows the matched preset with its emoji. After 800ms, the page redirects to `/?preset=<matched>`, opening the editor with the correct template pre-loaded. `⌘ Enter` keyboard shortcut for power users. Example prompt chips update the input and clear the match preview. "Browse gallery" and "Open blank editor" secondary CTAs added below examples. `useRouter` replaces the dead button stub. No AI backend needed — works instantly, correct match on first try for common sketch types (floor plans, flowcharts, offices, blueprints, gardens, technical diagrams, napkin sketches).
...
- [x] **wallWidth property on line elements — architectural wall rendering** — `LineElement` now has an optional `wallWidth?: number` field. When set to any positive value, the line renders as two parallel hand-drawn wobble lines with a filled interior — the standard double-line representation used in architectural floor plans. The fill uses the element's `fillColor` if set, otherwise adopts the current paper background color so walls blend naturally. The fill + two wobble lines are computed via perpendicular offset math (unit normal vector scaled by ±wallWidth/2). Both the canvas renderer (`renderer.ts`) and SVG exporter (`svg-renderer.ts`) implement identical wall geometry. Hit-testing and selection bounding boxes in `Canvas.tsx` now expand to cover the full wall thickness. UI: a new "Wall Thickness" section appears in the sidebar Selection panel whenever one or more line elements are selected — with a slider (2–30px), 5 quick-preset buttons (— plain, 4 thin, 8 interior, 12 exterior, 20 thick), and a "✕ plain line" clear button. Fully undo/redo integrated via `updateElement → pushHistory`. Wired in both `/` and `/editor` pages, desktop sidebar and mobile bottom-sheet ControlPanel. Removes the need to manually duplicate lines at +7px offset to simulate walls in presets.
